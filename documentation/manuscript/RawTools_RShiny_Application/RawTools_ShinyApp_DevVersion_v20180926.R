...
ui = shinyUI(
  ...
  dashboardPage(
    ...
    dashboardSidebar(
      sidebarMenu(
        menuItem("Scan Matrix", tabName = "scan_tab", icon = icon("th")),
        menuItem("Quality Control", tabName = "qc_tab", icon = icon("dashboard"))
      )
    ),
    dashboardBody(
      tabItems(
                         ...
                         tabBox(title = "1. Data Input", id = "scan_tabset1", width = NULL, side = 'right',  
                                ...
                                tabPanel("Example Data",
                                         ...
                                         tags$div(class="header", checked=NA,
                                                  tags$a(href="https://github.com/kevinkovalchik/RawTools/blob/master/documentation/QC_example-data/ScanData_Matrix.txt", 
  ...
  output$scan_plots = renderPlot({
    ...
    if (input$scan_xaxis_radio == 1){
      ...
      if (input$scan_plot_id == 2) {
        ...
        minBorder = 0
        maxBorder = round(max(log10(scan_data[,'BaseLinePeakWidth.s.']), na.rm = TRUE) + (max(log10(scan_data[,'BaseLinePeakWidth.s.']), na.rm = TRUE)*0.1), 2) 
        intBorder = round((max(log10(scan_data[,'BaseLinePeakWidth.s.']), na.rm = TRUE) / 10), 2)
        ...
        minBorder = 0
        maxBorder = round(max(log10(scan_data[,'MS1IonInjectionTime']), na.rm = TRUE) + (max(log10(scan_data[,'MS1IonInjectionTime']), na.rm = TRUE)*0.1), 2) 
        intBorder = round((max(log10(scan_data[,'MS1IonInjectionTime']), na.rm = TRUE) / 10), 2)
        ...
        minBorder = 0
        maxBorder = round(max(log10(scan_data[,'MS2IonInjectionTime']), na.rm = TRUE) + (max(log10(scan_data[,'MS2IonInjectionTime']), na.rm = TRUE)*0.1), 2) 
        intBorder = round((max(log10(scan_data[,'MS2IonInjectionTime']), na.rm = TRUE) / 10), 2)
