...
ui = shinyUI(fluidPage(
  title = "RawTools",
  h2('Data Plotting for RawTools Diagnostic QC Output'),
  ...
  fluidRow(
    ...
    column(7,
           h5('Example Data'),
           helpText('No data set yet? Download the example data. This is a set of 140 replicate injections of a HeLa digest on an Orbitrap Velos system. The data were searched using IdentiPy in RawTools, where oxidation of methionine was used as a variable modification with -xmod.'),
           br(),
  ...
  br(),
    ...
    column(3,
           ...
           helpText('Adjust the values to set the range of the x-axis. The values will set the minimum, maximum, and the interval between tick marks on the x-axis. If you only want to display a specific set of samples, use these values to adjust which are displayed. The values in the boxes are the preset defaults. These values can be changed in real-time and the plots will update.'),
           fluidRow(
             column(4,
                    h6('min'),
    ...
    column(1),
    column(3,
           h4('3. Optional: input custom values for the y-axis.'),
           helpText('Adjust the values to set the range of the y-axis. The values will set the minimum, maximum, and the interval between tick marks on the y-axis. Select the Use Custom Values radio button to activate your input values. The values in the boxes are the preset defaults.'),
    ...
    column(1),
    column(3,
           h4('4. Optional: input values to use for marking outliers.'),
           helpText('Input an optimum value and the percentage of difference from this value you consider to be acceptable. Data points that extend beyond the allowable percentage of the input optimum value will be marked as outliers on the generated plots.'),
...
server = shinyServer(function(input, output) {
  ...
  output$qc_plots = renderPlot({
    ...
    if ((input$yaxis_radio == 1) & (input$outlier_radio == 1)){
      ...
      if (input$qc_plot_id == 2) {
        ...
        minBorder = 0
        maxBorder = round(max(qc_data[xaxis_first:xaxis_last,'Ms1MedianSummedIntensity'], na.rm = TRUE) + (max(qc_data[xaxis_first:xaxis_last,'Ms1MedianSummedIntensity'], na.rm = TRUE)*0.1), 2) 
        intBorder = round((max(qc_data[xaxis_first:xaxis_last,'Ms1MedianSummedIntensity'], na.rm = TRUE) / 10), 5)
        ...
        minBorder = 0
        maxBorder = round(max(qc_data[xaxis_first:xaxis_last,'Ms2MedianSummedIntensity'], na.rm = TRUE) + (max(qc_data[xaxis_first:xaxis_last,'Ms2MedianSummedIntensity'], na.rm = TRUE)*0.1), 2) 
        intBorder = round((max(qc_data[xaxis_first:xaxis_last,'Ms2MedianSummedIntensity'], na.rm = TRUE) / 10), 5)
        ...
        minBorder = 0
        maxBorder = round(max(qc_data[xaxis_first:xaxis_last,'MedianPrecursorIntensity'], na.rm = TRUE) + (max(qc_data[xaxis_first:xaxis_last,'MedianPrecursorIntensity'], na.rm = TRUE)*0.1), 2) 
        intBorder = round((max(qc_data[xaxis_first:xaxis_last,'MedianPrecursorIntensity'], na.rm = TRUE) / 10), 5)
    ...
    else if ((input$yaxis_radio == 1) & (input$outlier_radio == 2)){
      qcFile = input$qc_file
    ...
    else if ((input$yaxis_radio == 2) & (input$outlier_radio == 1)){
    ...
    else if ((input$yaxis_radio == 2) & (input$outlier_radio == 2)){
