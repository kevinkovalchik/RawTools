...
namespace RawTools
{
    class Program
    {
        ...
        static int DoStuff(ArgumentParser.UniversalOptions opts)
        {
            List<string> files = new List<string>();
            ...
            else // did the user give us a directory?
            {
                ...
                if (opts.QcDirectory != null)
                {
                    (files, qcDataCollection) = QcWorkflow.GetFileListAndQcFile(parameters);
                }

                // if not, the parse method
                else if (Directory.Exists(opts.InputDirectory))
                {
                    files = Directory.GetFiles(opts.InputDirectory, "*.*", SearchOption.TopDirectoryOnly)
                    .Where(s => s.EndsWith(".raw", StringComparison.OrdinalIgnoreCase)).ToList();
                }
