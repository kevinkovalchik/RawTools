...
namespace RawTools.ArgumentParser
{
    ...
    [Verb("all", HelpText = "Combines qc and parse options into one workflow, allowing for simultaneous use. QC is invoked by inclusion of " +
        "the \"-Q\" parameter (indicates the directory to hold the QC data). There is currently very little error checking of parameters " +
        "using the mode, so be cautious.")]
    class UniversalOptions
    {
        ...
        [Option('Q', "qcdirectory", HelpText = "Path to the directory containing (or to contain) the QC data file (called QC.xml)")]
        public string QcDirectory { get; set; }
